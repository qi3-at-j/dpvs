...
cur_conf_dir=$cur_work_dir/conf
cur_script_dir=$cur_work_dir/script
cur_bin_dir=$cur_sys_dir/$cur_kernel/bin
cur_ko_dir=$cur_sys_dir/$cur_kernel/ko
cur_lib_dir=$cur_sys_dir/$cur_kernel/lib
cur_ips_conf_dir=$cur_work_dir/ips_conf
cur_ips_bin_dir=$cur_sys_dir/$cur_kernel/ips_bin
work_conf_dir=/etc
cur_ips_tools_dir=$cur_work_dir/ips_tools
...
set_flow_env()
{
	...
	if [ "$?" != "0" ]; then
		...
		echo "set_dev success"
	fi

	echo "set_hugepages start..."
	set_hugepages
	if [ "$?" != "0" ]; then
		echo "set_hugepages err!!!"
		return 1
	else
		echo "set_hugepages success"
	fi

	echo "set_sys_env start..."
	set_sys_env
	if [ "$?" != "0" ]; then
		echo "set_sys_env err!!!"
		return 1
	else
		echo "set_sys_env success"
	fi
...
set_ips_conf()
{
	if [ ! -d "$cur_ips_conf_dir" ]; then
		echo "$cur_ips_conf_dir does not exist!!!"
		return 1
	fi

    mkdir -p $work_conf_dir/suricata/
	cd $cur_ips_conf_dir
	\cp -rf * $work_conf_dir/suricata/

	cd - > /dev/null
	
	if [ ! -d "$cur_ips_bin_dir" ]; then
		echo "$cur_ips_bin_dir does not exist!!!"
		return 1
	fi
...
set_ips_env()
{
	echo "set_ips_conf start..."
	set_ips_conf
	if [ "$?" != "0" ]; then
		echo "set_ips_conf err!!!"
		return 1
	else
		echo "set_ips_conf success"
	fi

	echo "$cur_lib_dir" | sudo tee --append /etc/ld.so.conf.d/usrlocal.conf
	ldconfig

        echo "set ips tools start..."
        cd $cur_ips_tools_dir
        python setup.py build
        python setup.py install
        cd - > /dev/null
        if [ -e "/usr/bin/suricatasc" ]
            then
            echo "set ips tools success"
            return 0
        else 
            echo "set ips tools failed"
            return 1
        fi
}
...
case $1 in
	-s|-set)
		...
		echo "Set ips env start..."
		set_ips_env
		if [ "$?" != "0" ]; then
			echo "Set ips env err!!!"
			exit 1
		fi
		echo "Set ips env success"
		;;
	-u|-unset)
