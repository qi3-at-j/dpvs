                              ...
                              struct rte_mbuf *mbuf)
{
     ...
     */
    if (unlikely(mbuf_userdata_get(mbuf) != NULL)) {
        RTE_LOG(WARNING, IPVS, "%s: FNAT have route %p ?\n",
                __func__, mbuf_userdata_get(mbuf));
        route4_put((struct route_entry *)mbuf_userdata_get(mbuf));
    }
                              ...
                              struct rte_mbuf *mbuf)
{
    ...
    if (unlikely(mbuf_userdata_get(mbuf) != NULL)) {
        RTE_LOG(WARNING, IPVS, "%s: FNAT have route %p ?\n",
                __func__, mbuf_userdata_get(mbuf));
        route6_put((struct route6 *)mbuf_userdata_get(mbuf));
    }
                            ...
                            struct rte_mbuf *mbuf)
{
    ...
    if (unlikely(mbuf_userdata_get(mbuf) != NULL)) {
        RTE_LOG(WARNING, IPVS, "%s: Already have route %p ?\n",
                __func__, mbuf_userdata_get(mbuf));
        route6_put(mbuf_userdata_get(mbuf));
    }
                                ...
                                struct rte_mbuf *mbuf)
{
    ...
    if (rt->port && rt->port->offload & NETIF_PORT_TX_IP_CSUM_OFFLOAD) {
