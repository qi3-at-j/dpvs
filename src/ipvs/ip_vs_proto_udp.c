    ...
    if (AF_INET6 == af) {
        ...
        if (unlikely(opp != NULL)) {
            ...
            if (rt6 && rt6->rt6_dev)
                ...
                dev = conn->out_dev;
            if (likely(dev && (dev->offload & NETIF_PORT_TX_UDP_CSUM_OFFLOAD))) {
                               ...
                               enum uoa_mode mode)
{
    ...
    assert(conn && ombuf && oiph && ouh && mbuf_userdata_get(ombuf));
    ...
    if (AF_INET6 == oaf) {
        ...
        rt6 = (struct route6*)mbuf_userdata_get(ombuf);
        ...
        rt = (struct route_entry *)mbuf_userdata_get(ombuf);
