...
/* 解析Fsbuf，获取四层偏移 */
STATIC UINT SESSION6_kGetL4OffsetFromFsbuf(IN UINT uiL3Offset, IN const MBUF_S *pstMBuf)
{
	...
	pstIP6Hdr= MBUF_BTOD_OFFSET(pstMBuf,uiL3Offset,IP6_S*);

	ucNxtHdr = pstIP6Hdr->ip6_ucNxtHdr;
	uiTotLen = ntohs(pstIP6Hdr->ip6_usPLen);
	uiLenTraversed =(UINT)sizeof(IP6_S)+ uiL3Offset;

	uiL4Offset = uiL3Offset + (UINT)sizeof(IP6_S);
	...
	while (uiTotLen > (UINT)sizeof(struct ip6_ext))
	{
		...
		pstExtHdr = MBUF_BTOD_OFFSET(pstMBuf, uiLenTraversed, VOID*);
...
STATIC inline VOID session6_FsTcpStateIV(IN SESSION_CTRL_S *pstSessionCtrl,
                                         IN SESSION_S *pstSession,
                                         IN MBUF_S *pstMBuf,
                                         IN UINT uiL4Offset,
                                         IN SESSION_PKT_DIR_E enDir)
...
/* 快转tcp状态机处理 */
STATIC inline VOID SESSION6_FsTcpState(IN SESSION_S *pstSession,
									   IN MBUF_S *pstMBuf,
									   IN SESSION_PKT_DIR_E enDir, 
                                       IN SESSION_CTRL_S *pstSessionCtrl,
									   IN UINT uiL3Offset) 
{
	...
	/* 进行单包合法性检查 */ 
	uiL4Offset = SESSION6_kGetL4OffsetFromFsbuf(uiL3Offset, pstMBuf);
	pstTcpHdr = MBUF_BTOD_OFFSET(pstMBuf, uiL4Offset, TCPHDR_S *);
	ucFlags =(pstTcpHdr->th_flags) & TCP_FLAGS_CARE_MASK;
	...
	switch (ucNewState)
	{
		case sTCP_IV:
		{
			session6_FsTcpStateIV(pstSessionCtrl,pstSession,pstMBuf,uiL4Offset,enDir);
			break; 
		}
		case sTCP_IG:
		{
