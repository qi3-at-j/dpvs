...
const char *log_file_backup_name = "/var/log/ctyun_fw_backup.log";
int log_file_max_size = 1; /* 1k */
int log_file_size_check_interval = 5;  /* 5seconds*/
const char *log_file_name = "/var/log/ctyun_fw.log";
static FILE *log_file;
bool always_flush_log_file = false;
#endif
...
int 
log_file_get_size(void)
{
    char *log_file_full_name;
    struct stat statbuf; 

    log_file_full_name = make_file_name(log_file_name, "vrrp", NULL, NULL);
    if (log_file_full_name)
    {
        stat(log_file_full_name, &statbuf); 
