    ...
    while (cpu_left > 0) {
        ...
        list_for_each_entry_reverse(queue, &worker_min->port_list, queue_list_node) {
            ...
            for (ii = 0; queue->rx_queues[ii] != NETIF_MAX_QUEUES && ii < NETIF_MAX_QUEUES;
				...
				lcore_conf[id].rx_queue_list[ln].port_id = pid; //lcore_conf下初始化port-id
				snprintf(lcore_conf[id].rx_queue_list[ln].node_name, RTE_NODE_NAMESIZE, 
					"ethdev_rx-%u-%u", pid, queue->rx_queues[ii]);
				lcore_conf[id].rx_queue_list[ln++].queue_id = queue->rx_queues[ii]; //lcore_conf下初始化queue-id
		...
		lcore_conf[id].n_rx_queue = ln; //统计在该lcore下的所有接收队列数。
        id++;
...
{
	/*lcore2index[0]初始化无效是因为第一个通常是master。
	例如
	lcore_id :  0 1 2 3 4 5 6
	index :     7 0 1 2 3 4 5 7 
	因此，lcore_conf[0]保存的是lcore_id为1线程的值
	      lcore_conf[1]保存的是lcore_id为2线程的值
	*/
    lcoreid_t ii;
    int tk = 0;
...
void netif_get_slave_lcores(uint8_t *nb, uint64_t *mask)
{
    ...
    while (lcore_conf[i].nports > 0) {
         ...
         * as ports is configured for KNI core.
         应该排除LCORE_ROLE_KNI_WORKER，因为端口是为KNI core配置的。 */
...
netif_init_graph_need_to_create(void)
{
	...
	for (lcore_id = 0; lcore_id < DPVS_MAX_LCORE; lcore_id++) {
		...
		if ((qconf->type == LCORE_ROLE_FWD_WORKER)&&(qconf->n_rx_queue))
			nb_graphs++;//这个应该是lcore的个数
	}

	printf("graph number to create : %d\n", nb_graphs);
}

//经netif_init_graph_need_to_create初始化全局变量nb_graphs方可用
uint16_t 
...
init_rte_node_ethdev_config(portid_t pid){
	...
	if(dev){
		...
		ethdev_conf[nb_conf].mp_count = 1;	//暂时先写死
	}
...
init_graph_for_per_slave_core(void)
{
	...
	for (lcore_id = 0; lcore_id < DPVS_MAX_LCORE; lcore_id++) {
		...
		/* Skip graph creation if no source exists & 不是转发核略过*/
...
static int check_lcore_conf(int lcores, const struct netif_lcore_conf *lcore_conf)
{
    ...
    while (lcore_conf[i].nports > 0) {
		...
		//杩欎釜闂鐨勫彂鐜拌繃绋嬪強鍏舵伓蹇冦�傛澶勫幓鎺夛紝鍥犱负锛宒pvs闄愬埗浜嗘瘡涓猯core閮借鏈夋墍鏈夌殑缃戝崱闃熷垪锛岀己涓�涓嶅彲锛屼絾鏄垜浠兂瑕佹洿鐏垫椿銆傚幓鎺夎繖涓檺鍒讹紒锛?		
    ...
    if (pt->type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP) && !pkts_from_ring) {
        ...
        if (rte_be_to_cpu_16(arp->arp_opcode) == RTE_ARP_OP_REPLY) {
            for (i = 0; i < DPVS_MAX_LCORE; i++) {
				...
				//此处还可以优化，比如arp_ring所在的lcore如果不是和本地lcore在一个socket上的话
				//会有跨核访问的事情发生。但是即使此处mbuf_pool用的是ring所在的socket，这在申请内存的
				//时候就跨socket了，所以是不可以避免的吗？？
...
static inline void setup_dev_of_flags(struct netif_port *port)
{
     ...
     * or we have to check if VID is PVID (than to tx offload it).
	     鎴戜滑鍙兘鍦ㄤ竴涓猺te_ethdev涓婃湁澶氫釜vlan dev锛岃�宮buf->vlan_tci鏄疪X !
	     铏界劧鍙湁涓�涓狿VID (DEV_TX_OFFLOAD_VLAN_INSERT)锛屼负浜嗘柟渚胯捣瑙侊紝
	     涓嶆敮鎸乀X VLAN鎻掑叆鍗歌浇銆傛垨鑰呮垜浠繀椤绘鏌ID鏄惁涓篜VID(姣斿彂閫佸嵏杞藉畠)銆?     */
#if 0
...
int netif_port_list_rcu_lock_init(){
	...
	/*1.因为fdb的释放采用无锁flag，写者需要延迟释放提高效率，所以需要延迟队列+qsbr组合来模拟rcu。此处是申请fdb qsbr，并初始化，运行在
	 general_rcu的rcu start在转发核全部进入循环后统一触发执行。*/
