    ...
    while (cpu_left > 0) {
        ...
        list_for_each_entry_reverse(queue, &worker_min->port_list, queue_list_node) {
            ...
            for (ii = 0; queue->rx_queues[ii] != NETIF_MAX_QUEUES && ii < NETIF_MAX_QUEUES;
				...
				lcore_conf[id].rx_queue_list[ln].port_id = pid; //lcore_confœ¬≥ı ºªØport-id
				snprintf(lcore_conf[id].rx_queue_list[ln].node_name, RTE_NODE_NAMESIZE, 
					"ethdev_rx-%u-%u", pid, queue->rx_queues[ii]);
				lcore_conf[id].rx_queue_list[ln++].queue_id = queue->rx_queues[ii]; //lcore_confœ¬≥ı ºªØqueue-id
		...
		lcore_conf[id].n_rx_queue = ln; //Õ≥º∆‘⁄∏√lcoreœ¬µƒÀ˘”–Ω” ’∂”¡– ˝°£
        id++;
...
{
	/*lcore2index[0]≥ı ºªØŒﬁ–ß «“ÚŒ™µ⁄“ª∏ˆÕ®≥£ «master°£
	¿˝»Á
	lcore_id :  0 1 2 3 4 5 6
	index :     7 0 1 2 3 4 5 7 
	“Ú¥À£¨lcore_conf[0]±£¥Êµƒ «lcore_idŒ™1œﬂ≥Ãµƒ÷µ
	      lcore_conf[1]±£¥Êµƒ «lcore_idŒ™2œﬂ≥Ãµƒ÷µ
	*/
    lcoreid_t ii;
    int tk = 0;
...
void netif_get_slave_lcores(uint8_t *nb, uint64_t *mask)
{
    ...
    while (lcore_conf[i].nports > 0) {
         ...
         * as ports is configured for KNI core.
         ”¶∏√≈≈≥˝LCORE_ROLE_KNI_WORKER£¨“ÚŒ™∂Àø⁄ «Œ™KNI core≈‰÷√µƒ°£ */
...
netif_init_graph_need_to_create(void)
{
	...
	for (lcore_id = 0; lcore_id < DPVS_MAX_LCORE; lcore_id++) {
		...
		if ((qconf->type == LCORE_ROLE_FWD_WORKER)&&(qconf->n_rx_queue))
			nb_graphs++;//’‚∏ˆ”¶∏√ «lcoreµƒ∏ˆ ˝
	}

	printf("graph number to create : %d\n", nb_graphs);
}

//æ≠netif_init_graph_need_to_create≥ı ºªØ»´æ÷±‰¡ønb_graphs∑Ωø…”√
uint16_t 
...
init_rte_node_ethdev_config(portid_t pid){
	...
	if(dev){
		...
		ethdev_conf[nb_conf].mp_count = 1;	//‘› ±œ»–¥À¿
	}

	
	nb_conf++;//ø…º˚∏√÷µ «Õ¯ø®µƒ∏ˆ ˝
...
init_graph_for_per_slave_core(void)
{
	...
	for (lcore_id = 0; lcore_id < DPVS_MAX_LCORE; lcore_id++) {
		...
		/* Skip graph creation if no source exists & ≤ª «◊™∑¢∫À¬‘π˝*/
		...
		//ÊâìÂç∞Âõæ‰∏ãÈù¢ÁöÑÊâÄÊúâËäÇÁÇπÔºå‰ª•Âèä‰ªñÁöÑËæ?		rte_graph_node_printf(qconf->graph, true);
	}
...
static int check_lcore_conf(int lcores, const struct netif_lcore_conf *lcore_conf)
{
    ...
    while (lcore_conf[i].nports > 0) {
		...
		//Ëøô‰∏™ÈóÆÈ¢òÁöÑÂèëÁé∞ËøáÁ®ãÂèäÂÖ∂ÊÅ∂ÂøÉ„ÄÇÊ≠§Â§ÑÂéªÊéâÔºåÂõ†‰∏∫ÔºådpvsÈôêÂà∂‰∫ÜÊØè‰∏™lcoreÈÉΩË¶ÅÊúâÊâÄÊúâÁöÑÁΩëÂç°ÈòüÂàóÔºåÁº∫‰∏Ä‰∏çÂèØÔºå‰ΩÜÊòØÊàë‰ª¨ÊÉ≥Ë¶ÅÊõ¥ÁÅµÊ¥ª„ÄÇÂéªÊéâËøô‰∏™ÈôêÂà∂ÔºÅÔºÅ
    ...
    if (pt->type == rte_cpu_to_be_16(RTE_ETHER_TYPE_ARP) && !pkts_from_ring) {
        ...
        if (rte_be_to_cpu_16(arp->arp_opcode) == RTE_ARP_OP_REPLY) {
            for (i = 0; i < DPVS_MAX_LCORE; i++) {
				...
				//¥À¥¶ªπø…“‘”≈ªØ£¨±»»Áarp_ringÀ˘‘⁄µƒlcore»Áπ˚≤ª «∫Õ±æµÿlcore‘⁄“ª∏ˆsocket…œµƒª∞
				//ª·”–øÁ∫À∑√Œ µƒ ¬«È∑¢…˙°£µ´ «º¥ π¥À¥¶mbuf_pool”√µƒ «ringÀ˘‘⁄µƒsocket£¨’‚‘⁄…Í«Îƒ⁄¥Êµƒ
				// ±∫ÚæÕøÁsocket¡À£¨À˘“‘ «≤ªø…“‘±‹√‚µƒ¬£ø£ø
...
static void lcore_job_recv_fwd(void *arg)
{
	...
	//ÊöÇÊó∂ÂÖàÂéªÊéâËøôÈÉ®ÂàÜÂäüËÉΩÔºåÁî®ÂõæËäÇÁÇπÊé•Êî∂
	return;
...
static inline void setup_dev_of_flags(struct netif_port *port)
{
     ...
     * or we have to check if VID is PVID (than to tx offload it).
	     Êàë‰ª¨ÂèØËÉΩÂú®‰∏Ä‰∏™rte_ethdev‰∏äÊúâÂ§ö‰∏™vlan devÔºåËÄåmbuf->vlan_tciÊòØRX !
	     ËôΩÁÑ∂Âè™Êúâ‰∏Ä‰∏™PVID (DEV_TX_OFFLOAD_VLAN_INSERT)Ôºå‰∏∫‰∫ÜÊñπ‰æøËµ∑ËßÅÔºå
	     ‰∏çÊîØÊåÅTX VLANÊèíÂÖ•Âç∏ËΩΩ„ÄÇÊàñËÄÖÊàë‰ª¨ÂøÖÈ°ªÊ£ÄÊü•VIDÊòØÂê¶‰∏∫PVID(ÊØîÂèëÈÄÅÂç∏ËΩΩÂÆÉ)„ÄÇ
     */
#if 0
	...
	//Â¶ÇÊûúËÆæÂ§áÊîØÊåÅvlan filterÔºåÂàôËÆæÁΩÆÊ†áÂøó.
