...
int
tyflow_cirbuf_init(struct cirbuf *cbuf, char *buf, unsigned int start, unsigned int maxlen)
{
	if (!cbuf || !buf)
		return -EINVAL;
	cbuf->maxlen = maxlen;
...
int
tyflow_cirbuf_add_buf_head(struct cirbuf *cbuf, const char *c, unsigned int n)
{
	unsigned int e;
...
int
tyflow_cirbuf_add_buf_tail(struct cirbuf *cbuf, const char *c, unsigned int n)
{
	unsigned int e;
...
int
tyflow_cirbuf_add_head_safe(struct cirbuf * cbuf, char c)
{
...
void
tyflow_cirbuf_add_head(struct cirbuf * cbuf, char c)
{
	__cirbuf_add_head(cbuf, c);
}
...
int
tyflow_cirbuf_add_tail_safe(struct cirbuf * cbuf, char c)
{
	if (cbuf && !CIRBUF_IS_FULL(cbuf)) {
...
void
tyflow_cirbuf_add_tail(struct cirbuf * cbuf, char c)
{
	__cirbuf_add_tail(cbuf, c);
}
...
int
tyflow_cirbuf_align_left(struct cirbuf * cbuf)
{
	if (!cbuf)
		return -EINVAL;
...
/* XXX we could do a better algorithm here... */
int
tyflow_cirbuf_align_right(struct cirbuf * cbuf)
{
	if (!cbuf)
		return -EINVAL;
...
int
tyflow_cirbuf_del_buf_head(struct cirbuf *cbuf, unsigned int size)
{
...
int
tyflow_cirbuf_del_buf_tail(struct cirbuf *cbuf, unsigned int size)
{
	if (!cbuf || !size || size > CIRBUF_GET_LEN(cbuf))
		return -EINVAL;
...
int
tyflow_cirbuf_del_head_safe(struct cirbuf * cbuf)
{
...
void
tyflow_cirbuf_del_head(struct cirbuf * cbuf)
{
	__cirbuf_del_head(cbuf);
}
...
int
tyflow_cirbuf_del_tail_safe(struct cirbuf * cbuf)
{
	if (cbuf && !CIRBUF_IS_EMPTY(cbuf)) {
...
void
tyflow_cirbuf_del_tail(struct cirbuf * cbuf)
{
	__cirbuf_del_tail(cbuf);
}
...
int
tyflow_cirbuf_get_buf_head(struct cirbuf *cbuf, char *c, unsigned int size)
{
	unsigned int n;
...
int
tyflow_cirbuf_get_buf_tail(struct cirbuf *cbuf, char *c, unsigned int size)
{
	unsigned int n;
...
char
tyflow_cirbuf_get_head(struct cirbuf * cbuf)
{
...
char
tyflow_cirbuf_get_tail(struct cirbuf * cbuf)
{
