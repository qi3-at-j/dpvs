 ...
 */
int tyflow_cirbuf_init(struct cirbuf *cbuf, char *buf, unsigned int start, unsigned int maxlen);
 ...
 */
int tyflow_cirbuf_add_head_safe(struct cirbuf *cbuf, char c);
 ...
 */
int tyflow_cirbuf_add_buf_head(struct cirbuf *cbuf, const char *c, unsigned int n);
 ...
 * copied bytes on success, or a negative value on error.
 */
int tyflow_cirbuf_add_buf_tail(struct cirbuf *cbuf, const char *c, unsigned int n);

/**
 * Remove chars at the head of the circular buffer. Return 0 on
 * success, or a negative value on error.
 */
int tyflow_cirbuf_del_buf_head(struct cirbuf *cbuf, unsigned int size);

/**
 * Remove chars at the tail of the circular buffer. Return 0 on
 * success, or a negative value on error.
 */
int tyflow_cirbuf_del_buf_tail(struct cirbuf *cbuf, unsigned int size);
 ...
 * chars.
 */
int tyflow_cirbuf_get_buf_head(struct cirbuf *cbuf, char *c, unsigned int size);
 ...
 * chars.
 */
int tyflow_cirbuf_get_buf_tail(struct cirbuf *cbuf, char *c, unsigned int size);


/**
 * Set the start of the data to the index 0 of the internal buffer.
 */
int tyflow_cirbuf_align_left(struct cirbuf *cbuf);

/**
 * Set the end of the data to the last index of the internal buffer.
 */
int tyflow_cirbuf_align_right(struct cirbuf *cbuf);
