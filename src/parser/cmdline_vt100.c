...
void
tyflow_vt100_init(struct cmdline_vt100 *vt)
{
	if (!vt)
		return;
...
match_command(char *buf, unsigned int size)
{
	...
	for (i=0 ; i<sizeof(tyflow_cmdline_vt100_commands)/sizeof(const char *) ; i++) {
		cmd = *(tyflow_cmdline_vt100_commands + i);
...
int
tyflow_vt100_parser(struct cmdline_vt100 *vt, char ch)
{
	unsigned int size;
