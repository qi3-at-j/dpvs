...
#include "neigh.h"
#include "scheduler.h"
#include "../include/l3_temp.h"
#include "../include/l2_debug.h"

/* Next node for each ptype, default is '0' is "pkt_drop" */

static inline uint16_t l3_temp_to_next_index(s_nc_param *param)
{
	...
	*/
	uint16_t index = L3_TEMP_NEXT_PKT_DROP;
	struct rte_mbuf *mbuf = param->mbuf;

	struct netif_port *dev = mbuf_dev_get(mbuf);
	struct netif_port *to  = dev;
	if(unlikely(!dev)){
		debug_l2_packet_trace(L2_DEBUG_L3_TEMP, mbuf, DETAIL_OFF, "[L3_temp]mbuf dev = null\n");
		return index;
	}

	if(dev->type == PORT_TYPE_GENERAL){
		if(unlikely(mbuf->port >= ether_output_real_port_cnt_get()) )
		{
			debug_l2_packet_trace(L2_DEBUG_L3_TEMP, mbuf, DETAIL_OFF, "[L3_temp]mbuf which is from real-dev 's port-id = %d > real port cnt\n", mbuf->port);
			return index;
		}
	}
		
	return L3_TEMP_NEXT_IPV4;		
}
static uint16_t
l3_temp_node_process(struct rte_graph *graph, struct rte_node *node,
		     void **objs, uint16_t nb_objs)
{
	...
	ctx = (struct l3_temp_node_ctx *)node->ctx;
	...
	while (n_left_from >= 4) {
		 ...
		 */
	 	...
	 	n0 = l3_temp_to_next_index(&param[0]);
		n1 = l3_temp_to_next_index(&param[1]);
		n2 = l3_temp_to_next_index(&param[2]);
		n3 = l3_temp_to_next_index(&param[3]);
...
struct rte_node_register l3_temp_node = {
	.process = l3_temp_node_process,
	.name = "L3_temp",

	.nb_edges = L3_TEMP_NEXT_MAX,
	.next_nodes = {
		/* Pkt drop node starts at '0' */
		[L3_TEMP_NEXT_PKT_DROP] = "pkt_drop",
		//[ETHER_INPUT_NEXT_L2_BOND] = "bond",
		[L3_TEMP_NEXT_L2_XMIT] = "L2_xmit",
		[L3_TEMP_NEXT_IPV4]    = "ip4_rcv",
	},
};
		
RTE_NODE_REGISTER(l3_temp_node);
