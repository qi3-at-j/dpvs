...
static inline int get_netif_port_list(void)
{
    ...
    ret = get_port_list((void **)&p_port_list, &len);
...
static 
int set_port_cli(netif_nic_set_t *cfg){
    int ret = 0;
    struct netif_port *port;
    
    if (!cfg)
      return EDPVS_INVAL;
    port = netif_port_get_by_name(cfg->pname);
    if (!port)
      return EDPVS_INVAL;
    ret = set_port(port, cfg);
...
static int dump_nic_basic(char *name, int namelen)
{
    ...
    err = get_port_basic(port,  (void **)&p_get, &len);
...
static int dump_nic_stats(char *name, int namelen)
{
    ...
    err = get_port_stats(port, (void **)&p_get, &len);
    if (err != EDPVS_OK || !p_get || !len){
        printf("err = %d!\n", err);
        return err;
    }
...
static int dump_nic_verbose(char *name, int namelen)
{
    ...
    err = get_port_ext_info(port, (void **)&ext_get, &len);
...
static int dump_bond_status(char *name, int namelen)
{
    ...
    err = get_bond_status(port, (void **)(&p_get), &len);
    if (err != EDPVS_OK || !p_get || !len){
        printf("err = %d!\n", err);
        return err;
    }
...
static int dump_cpu_basic(lcoreid_t cid)
{
    ...
    if (!is_lcore_id_valid(cid))
        return EDPVS_INVAL;
    err = get_lcore_basic(cid, (void **)&p_get, &len);
    if (err != EDPVS_OK || !p_get || !len)
        return err;
...
static int dump_cpu_stats(lcoreid_t cid)
{
    ...
    if (!is_lcore_id_valid(cid)){
        printf("invaild cpu id!\n");
        return EDPVS_INVAL;
    }
             
    err = get_lcore_stats(cid, (void **)&p_get, &len);
    if (err != EDPVS_OK || !p_get || !len){
        printf("err = %d!\n", err);
        return err;
    }
    ...
    switch (param->dev_type) {
        ...
        case LINK_DEVICE_CPU:
        {
            ...
            if (param->dev_name[0]) { /* show information of specified CPU */
                ...
                err = get_lcore_mask((void **)&p_lcores, &len);
...
static int link_nic_set_promisc(const char *name, int value)
{
    netif_nic_set_t cfg;
    ...
    return set_port_cli(&cfg);
}

static int link_nic_set_forward2kni(const char *name, int value)
{
...
static int link_nic_set_link_status(const char *name, int value)
{
...
{
    struct link_param param; 
    snprintf(param.dev_name, sizeof(param.dev_name), "%s", cbt->string[0]);
    switch(cbt->which[0]){
        case 1:{
            //promisc
            link_nic_set_promisc(param.dev_name, cbt->which[1]);
            break;
        }
        case 2:{
            //forward2kni
            link_nic_set_forward2kni(param.dev_name, cbt->which[1]);
            break;
        }
        case 3:{
            //link
            link_nic_set_link_status(param.dev_name, cbt->which[1]);
            break;
        }
        default:{
...
static int
show_link_eol_cli(cmd_blk_t *cbt)
{
    int i;
    int err;
    struct link_param param;
    ...
    */
    ...
    if(cbt->which_cnt > 1){
        if(cbt->which[1] == 0){
            //sow interface name xxx | all -s
            param.stats.enabled = 1;
        }else if(cbt->which[1] == 1){
            //sow interface name xxx | all -v
            param.verbose = 1;
        }
    }
   
    if(cbt->which_cnt == 3){
        //sow interface name xxx | all -v -s
        param.stats.enabled = 1;
        param.verbose = 1;
    }

    if(cbt->number_cnt > 0){
        //sow interface name xxx | all [-v] -s -i xx
        param.stats.interval = cbt->number[0];
    }

    if(cbt->number_cnt == 2){
        //sow interface name xxx | all [-v] -s -i xx [-c xx]
        param.stats.count = cbt->number[1];
    }
    
    err = link_show(&param);
    if(err != EDPVS_OK){
        tyflow_cmdline_printf(cbt->cl, "err = %d!\n", err);
    }
	return 1;
}

EOL_NODE(interface_eol, set_interface_cli);
KW_NODE_WHICH(interface_link_down, interface_eol, none, "down", "make the interface link down", 2, 0);
KW_NODE_WHICH(interface_link_up, interface_eol, interface_link_down, "up", "make the interface link up", 2, 1);
KW_NODE_WHICH(interface_kni_off, interface_eol, none, "off", "forward2kni off", 2, 0);
KW_NODE_WHICH(interface_kni_on, interface_eol, interface_kni_off, "on", "forward2kni on", 2, 1);
KW_NODE_WHICH(interface_promisc_off, interface_eol, none, "off", "promisc off", 2, 0);
...
EOL_NODE(show_link_eol, show_link_eol_cli);
VALUE_NODE(show_cnt_num, show_link_eol, none, "the count number", 2, NUM);
KW_NODE(show_cnt, show_cnt_num, none, "-c", "the count");
VALUE_NODE(show_interval_time, show_cnt, none, "the interval time seconds", 1, NUM);
KW_NODE(show_interval, show_interval_time, show_link_eol, "-i", "the interval");
KW_NODE_WHICH(show_stats, show_interval, show_link_eol, "-s", "the stats of interface", 3, 1);
KW_NODE_WHICH(show_verbose, show_stats, show_stats, "-v", "the verbose of interface", 2, 1);
VALUE_NODE(show_link_one_name, show_verbose, none, "interface name", 1, STR);
KW_NODE_WHICH(show_link_one, show_link_one_name, none, "name", "the specific interface", 1, 2);
KW_NODE_WHICH(show_link_all, show_verbose, show_link_one, "all", "show all interface", 1, 1);
KW_NODE(show_interface, show_link_all, none, "interface", "show interface");

int
interface_cli_init(void)
{
    add_set_cmd(&cnode(interface));
    add_get_cmd(&cnode(show_interface));
    return 0;
}
