...
uint16_t vlan_graph_xmit(s_nc_param *param, struct rte_mbuf *mbuf, struct netif_port *dev)
{
	...
	mbuf_dev_set(mbuf, vlan->real_dev);
...
set_vlan_add_cli(cmd_blk_t *param){
	...
	struct vlan_dev_priv *vlan;
	int err;
	if(param->string_cnt > 2){
		tyflow_cmdline_printf(param->cl,  "invail \n");
		return EDPVS_INVAL;
	}

	char *vlan_sub_name = param->string[0];
	char *real_dev_name = param->string[1];
	uint16_t vlan_id = param->number[0];
	uint16_t vlan_proto = ETH_P_8021Q;

	if(!strlen(vlan_sub_name) || !strlen(real_dev_name)){
		tyflow_cmdline_printf(param->cl,  "invail name \n");
		return EDPVS_INVAL;
	}

	real_dev = netif_port_get_by_name(real_dev_name);
	if (!real_dev) {
		tyflow_cmdline_printf(param->cl,  "no such real device \n");
        return EDPVS_NODEV;
    }

	if (!vlan_id_valid(htons(vlan_id))) {
		tyflow_cmdline_printf(param->cl,  "invlid vlan ID \n");
		return EDPVS_INVAL;
	}

	if(param->mode == MODE_DO){
		err =  vlan_add_dev(real_dev, vlan_sub_name, htons(vlan_proto), htons(vlan_id));
		if(err != EDPVS_OK){
			tyflow_cmdline_printf(param->cl,  "vlan_add_dev failed, err = %d \n", err);
		}
	}else{
		dev = netif_port_get_by_name(vlan_sub_name);
        if (!dev || dev->netif_ops != &vlan_netif_ops){
			tyflow_cmdline_printf(param->cl,  "not such dev : %s \n", vlan_sub_name);
			return EDPVS_NOTEXIST;
		}

		vlan = netif_priv(dev);
        err = vlan_del_dev(vlan->real_dev,vlan->vlan_proto, vlan->vlan_id);
		if(err != EDPVS_OK){
			tyflow_cmdline_printf(param->cl,  "vlan_del_dev failed, err = %d \n", err);
		}
	}
...
/***************************vlan set***************************/
EOL_NODE_NEED_MAIN_EXEC(vlan_add_eol, set_vlan_add_cli);
VALUE_NODE(vlan_id_num, vlan_add_eol, none, "the vlan id num, between <1, 4096>", 1, NUM);
KW_NODE(vlan_id, vlan_id_num, none, "id", "the vlan id");
VALUE_NODE(link_name, vlan_id, none, "the port name, such as dpdk0.", 2, STR);
KW_NODE(link1, link_name, none, "link", "the port need to build vlan sub interface.");
VALUE_NODE(vlan_sub_name, link1, none, "the vlan sub interface name, such as dpdk0.100", 1, STR);
KW_NODE(vlan_set, vlan_sub_name, none, "vlan", "the vlan apps");

/***************************vlan get***************************
EOL_NODE_NEED_MAIN_EXEC(br_show_fdb_eol, show_fdb_entrys_cli);
EOL_NODE_NEED_MAIN_EXEC(br_show_detail_eol, br_detail_get_cli);
KW_NODE_WHICH(br_show_detail, br_show_detail_eol, none, "br-detail", "the bridge details as ports, packets etc.", 1, 2);
KW_NODE_WHICH(br_show_fdb, br_show_fdb_eol, br_show_detail, "fdb-entrys", "the bridge fdb hash entrys", 1, 1);
VALUE_NODE(br_show_name, br_show_fdb, none, "br-name", 1, STR);
...
KW_NODE_WHICH(brctl_all, debug_brctl_eol, none, "all", "enable/disable brctl all debug", 1, 4);
KW_NODE_WHICH(brctl_packet, debug_brctl_eol, brctl_all, "packet", "enable/disable brctl packet debug", 1, 3);
KW_NODE_WHICH(brctl_event, debug_brctl_eol, brctl_packet, "event", "enable/disable brctl event debug", 1, 2);
KW_NODE_WHICH(brctl_basic, debug_brctl_eol, brctl_event, "basic", "enable/disable brctl basic debug", 1, 1);
KW_NODE(vlan_debug, brctl_basic, none, "brctl", "enable/disable br app debug");
*/
static void
vlan_cli_init(void)
{
    add_set_cmd(&cnode(vlan_set));
	//add_get_cmd(&cnode(vlan_show));
	//add_debug_cmd(&cnode(vlan_debug));
}
