...
static inline enum vlan_node_next_nodes vlan_deal_and_trans_to_next_index(struct rte_mbuf *mbuf)
{
	...
	if(err != EDPVS_OK){
		return VLAN_NODE_NEXT_PKT_DROP;
	}

	//如果是桥接端口，则需要给bridge process进行处理。
	dev = mbuf_dev_get(mbuf);
	if(dev->br_port != NULL)
		return VLAN_NODE_NEXT_BRIDGE;

	//最后，没有特殊情况下，上送三层
	return VLAN_NODE_NEXT_L3_TEMP;
}
...
struct rte_node_register vlan_node = {
	...
	.next_nodes = {
		...
		[VLAN_NODE_NEXT_L3_TEMP] = "L3_temp",
		[VLAN_NODE_NEXT_BRIDGE] = "bridge",
	},
};
