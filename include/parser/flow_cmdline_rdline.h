 ...
 */
int tyflow_rdline_init(struct rdline *rdl,
		 rdline_write_char_t *write_char,
		 rdline_validate_t *validate,
		 rdline_complete_t *complete);
 ...
 */
void tyflow_rdline_newline(struct rdline *rdl, const char *prompt);

/**
 * Call it and all received chars will be ignored.
 * \param rdl A pointer to a struct rdline
 */
void tyflow_rdline_stop(struct rdline *rdl);
 ...
 * \param rdl A pointer to a struct rdline
 */
void tyflow_rdline_quit(struct rdline *rdl);

/**
 * Restart after a call to rdline_stop() or rdline_quit()
 * \param rdl A pointer to a struct rdline
 */
void tyflow_rdline_restart(struct rdline *rdl);

/**
 * Redisplay the current buffer
 * \param rdl A pointer to a struct rdline
 */
void tyflow_rdline_redisplay(struct rdline *rdl);

/**
 * Reset the current buffer and setup for a new line.
 *  \param rdl A pointer to a struct rdline
 */
void tyflow_rdline_reset(struct rdline *rdl);
 ...
 */
int tyflow_rdline_char_in(struct rdline *rdl, char c);

/**
 * Return the current buffer, terminated by '\0'.
 * \param rdl A pointer to a struct rdline
 */
const char *tyflow_rdline_get_buffer(struct rdline *rdl);
 ...
 */
int tyflow_rdline_add_history(struct rdline *rdl, const char *buf);

/**
 * Clear current history
 * \param rdl A pointer to a struct rdline
 */
void tyflow_rdline_clear_history(struct rdline *rdl);

/**
 * Get the i-th history item
 */
char *tyflow_rdline_get_history_item(struct rdline *rdl, unsigned int i);
