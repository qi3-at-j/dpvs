...
def get_v(rule_id,rule_act,t1,l1,v1):
    if t1 == 12 or t1 ==10 or t1 == 68 or t1 == 64 or t1 ==11 or t1 == 4 or t1 == 7 or t1 == 8 or t1 == 90:
        ...
        #return v1
        if t1 == 2:
            rule_act[rule_id]['name'] = str(v1)

        if t1 == 25:
            rule_act[rule_id]['name_cn'] = str(v1)

        if t1 == 3:
            rule_act[rule_id]['desc'] = str(v1)

        if t1 == 26:
            rule_act[rule_id]['desc_cn'] = str(v1)

        if t1 == 72:
            rule_act[rule_id]['cve'] = str(v1)

        if t1 == 86:
            rule_act[rule_id]['cnnvd'] = str(v1)
...
rule_act_info = str(rule_act)
rule_act_json = json.dumps(rule_act_info)
